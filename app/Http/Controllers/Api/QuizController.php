...
class QuizController extends Controller
{
    ...
    /**
     * Get all quizzes grouped by category (including scheduled quizzes)
    ...
    public function getAllQuizzes()
    {
        ...
        $quizzes = Cache::remember($cacheKey, 600, function () use ($user, $isPremium) {
            ...
            // NOW INCLUDING scheduled quizzes (is_active = true, regardless of publish_at)
            $quizzesBase = Cache::remember('quizzes_active_with_scheduled_ordered', 1800, function () {
                return Quiz::with('chapter')
                    ->leftJoin('chapters', 'quizzes.chapter_id', '=', 'chapters.id')
                    ->where('quizzes.is_active', true)
                    // REMOVED publish_at filter to include scheduled quizzes
            ...
            return $quizzesBase->map(function ($quiz) use ($isPremium) {
                ...
                $quiz->is_scheduled = $quiz->isScheduled(); // NEW: Frontend uses this for "Coming Soon" UI
                $quiz->publish_at_formatted = $quiz->publish_at ? $quiz->publish_at->format('Y-m-d H:i:s') : null; // NEW
                return $quiz;
            })->groupBy('category');
        });
    ...
    public function getQuizzesByCategory($category)
    {
        ...
        $quizzes = Cache::remember($cacheKey, 600, function () use ($category, $isPremium) {
            $quizzesBase = Quiz::with('chapter')
                ...
                ->where('quizzes.is_active', true)
                ->where(function ($query) {
                    $query->whereNull('quizzes.publish_at')
                        ->orWhere('quizzes.publish_at', '<=', now());
                })
                ->orderByRaw('COALESCE(quizzes.chapter_number, chapters.chapter_number, 999) ASC')
                ->orderBy('quizzes.id', 'asc')
                ->select('quizzes.*')
